...
<cfset variables.defaultQueryList = "title,description,link,date">
...
<cffunction name="getEntriesRSS" access="private" returnType="query" output="true"
	...
	<cfloop index="x" from="1" to="#arrayLen(items)#">
		...
		<!--- rss 0.92 allows enclosure and source --->
		<cfelseif structKeyExists(items[x],"enclosure") and structKeyExists(items[x].enclosure.XmlAttributes,"url")>
			<cfset node.link = items[x].enclosure.XmlAttributes.url>
